...
export type PartId = SynthPartId | DrumPartId | 'ACC';
...
export const ALL_PART_IDS: PartId[] = [...SYNTH_PART_IDS, ...DRUM_PART_IDS, 'ACC'];

export type Beat = '16' | '32' | '8T' | '16T';
export const BEATS: Beat[] = ['16', '32', '8T', '16T'];

/** Steps per bar for each beat mode (hardware: 16th / 32nd / 8-triplet / 16-triplet grids). */
export const STEPS_PER_BAR: Record<Beat, number> = { '16': 16, '32': 32, '8T': 12, '16T': 24 };
...
export const MAX_STEPS = 32 * MAX_BARS; // beat 32 x 8 bars = 256
...
export interface FxSlot {
  /** Effect type index 0..15 (see fx registry) */
  type: number;
  edit1: number; // 0..127
  edit2: number; // 0..127
...
export interface ArpSettings {
  /** Scale root note 0..11 (C..B) */
  key: number;
  /** Index into SCALES (params.ts / arp tables) */
  scale: number;
  /** Center note for the ribbon */
  centerNote: number;
}
...
export interface Song {
  name: string;
  /** If >0 overrides pattern tempo for the whole song. */
  tempo: number;
  events: SongEvent[];
}
...
export function stepsForPattern(p: Pick<Pattern, 'beat' | 'lengthBars'>): number {
  return STEPS_PER_BAR[p.beat] * p.lengthBars;
}
...
export function defaultSynthParams(): ParamMap {
  return {
    ...
    oscEdit2: 0,
    glide: 0,
    tune: 64, // 64 = center, +/-24 semitones mapped in registry
    filterType: 0,
    cutoff: 127,
    resonance: 0,
...
export function defaultDrumParams(waveId = 0): ParamMap {
  return {
    waveId,
    pitch: 64, // 64 = original pitch, +/-24 semitones
    filterType: 0,
    cutoff: 127,
    resonance: 0,
    egInt: 64, // bipolar, 64 = 0
...
export function createDefaultPattern(name = 'INIT'): Pattern {
  ...
  return {
    ...
    synths,
    drums,
    accent: { level: 64, steps: emptyDrumSteps() },
    fx: [
      { type: 2, edit1: 64, edit2: 64, chain: false },
      { type: 0, edit1: 64, edit2: 64, chain: false },
      { type: 5, edit1: 64, edit2: 64, chain: false },
    ],
    motions: [],
    arp: { key: 0, scale: 0, centerNote: 60 },
  };
}

export function createDefaultGlobal(): GlobalSettings {
  return { masterVolume: 100, valveGain: 0, tubeOn: true };
}
...
export function migratePattern(raw: unknown): Pattern {
  ...
  if (p.accent && typeof p.accent === 'object') {
    const src = p.accent as Partial<AccentPart>;
    if (typeof src.level === 'number') base.accent.level = src.level;
    if (Array.isArray(src.steps)) {
      src.steps.slice(0, MAX_STEPS).forEach((s, i) => {
        base.accent.steps[i] = { on: !!s.on };
      });
    }
  }
  ...
  if (p.arp && typeof p.arp === 'object') {
    ...
    base.arp = {
      key: typeof a.key === 'number' ? a.key : 0,
      scale: typeof a.scale === 'number' ? a.scale : 0,
        ...
        out.songs[i] = {
          ...
          events: s.events
            ...
            .map((e) => ({
              ...
              mutes: Array.isArray(e.mutes) ? e.mutes.filter((m): m is PartId => ALL_PART_IDS.includes(m as PartId)) : [],
            })),
        };
      }
    });
